...
#get route segment coords, then sample to get *ahem* a whole bunch more
...
NB_bi_direction_VMH_sf <- NB_VMH_sf %>%
  as.data.table(keep.rownames = T) %>%
  setkey(rn) %>%
  .[NB_bi_direction_closest_VMH_ind] %>%
  st_as_sf()

NB_none_VMH_sf <- NB_VMH_sf %>%
  as.data.table(keep.rownames = T) %>%
  setkey(rn) %>%
...
NB_bi_direction_VMH_sf %>% sample_n(5000) %>% leaflet %>% addTiles() %>% addFeatures()

NB_none_VMH_sf %>% sample_n(5000) %>% leaflet %>% addTiles() %>% addFeatures()


# snap points to line -----------------------------------------------------
rbindlist(list(NB_shared_center_VMH_sf,NB_bi_direction_VMH_sf,NB_none_VMH_sf),use.names = T,idcol = T)

gdata::combine(NB_shared_center_VMH_sf %>% head(),NB_none_VMH_sf %>% head())


NB_shared_center_VMH_sf %>%
  as.data.table() %>% #to get rid of slowness caused by sf
  .[, next_VMH_time := shift(Time,1,type = "lead"), .(Transit_Day,Vehicle_ID,Trip)] %>% 
  .[, .(time_diff = next_VMH_time - Time)]


leaflet() %>%
 addFeatures(SB_none_closest_VMH_sf[1824,],color = "blue") %>%
#  addFeatures(SB_shape_segments[3,],color = "red") %>% 
 addFeatures(SB_none_closest_VMH_sf[1825,],color = "black") %>% 
 addTiles()

points <- rbind(SB_none_closest_VMH_sf[1824,],
            SB_none_closest_VMH_sf[1825,])

rgeos::gProject(as(SB_shape_segments[3,],"Spatial"),as(points,"Spatial"),normalized = T)

points_proj <- points %>% st_transform(7328)
l_proj <- SB_shape_segments[3,] %>% st_transform(7328)
rgeos::gProject(as(l_proj,"Spatial"),as(points_proj,"Spatial"))


rgeos::gLength(as(l_proj,"Spatial"))
st_length(l_proj)*.4851933
